...
int my_getc(FILE* f) {
    ...
    int c = (unsigned char)**g_cursor;
    (*g_cursor)++;
    if (c == '\0') return EOF; // 0x00 corresponds to EOF in our model.
    return c;
}
...
struct token lex(struct p_state *state) {
    ...
    for (;;) {
        ...
        if (chr != ' ' && chr != '\n') {
            ...
            // If getc returns EOF, i.e. "-1", it will be stuck in an infinite loop, appending '-1' bytes.
