...
// Added for symbolic grammar mining: no comments.
static void skip_spaces(struct pstate *p) {
  const char *pos;
  do {
    pos = p->pos;
    while (mjs_is_space(p->pos[0])) {
...
static int pnext(struct pstate* p){
  ...
  skip_spaces(p); // no comments allowed
  p->tok.ptr = p->pos;
  p->tok.len = 1;
...
int kw_ep(int argc, char* argv[]) {
  ...
  int ret = parse_mjs(inp);
  if (ret) klee_silent_exit(1);

  return ret;
}

// tokenization proxy function (10 lines)
int sym_proxy_pnext(struct pstate *p) {
  p->tok.len = klee_int("token_length");
  klee_assume(p->tok.len <= 10);
  char* payload = malloc(11);
  klee_make_symbolic((void*)payload, 11, "token_payload");
  klee_assume(payload[10] == '\0');
  p->tok.ptr = (const char*)payload;
  p->prev_tok = p->tok.tok;
  p->tok.tok = stalagmite_get_token_by_id(p->pos++ - p->buf);
  return p->tok.tok; }
...
int kw_pnext(int argc, char* argv[]) { 
  ...
  p.mjs = mjs_create(); 
  pnext(&p); 
  klee_pass_token(klee_get_valuel(p.tok.tok));
